...
**绝对不要读 `browse.html`** —— 图片全部内联在里面，读进来直接撑爆 context。
CLI 结尾打印的 `open 'browse.html'` 是说给用户的，不是给你的。
...
命令输出里有一行 `保留 N 帧 (压缩率 X%)`。**这个百分比是质量信号，跑完先看它。**

| 压缩率 | 含义 | 动作 |
|---|---|---|
| < 15% | 正常。录屏、幻灯片、演示类内容 | 直接读 |
| 15–30% | 偏密，可接受 | 直接读，或调疏一档 |
| **> 30%** | **去重失效了** | **提高 threshold 重跑** |

压缩率过高几乎只有一个原因：**手持实拍**。镜头轻微抖动 + 人脸和肢体持续微动，
会让每一帧的感知哈希都算「不同」，于是同一个镜头被拆成几十张几乎一样的图 —— 白烧 token。

实测一个 19 秒手持自拍视频：

```
threshold 10 → 20 帧 (52.6%)   同一镜头被拆成 20 张，无意义
threshold 20 →  3 帧 ( 7.9%)   正确：单镜头视频就该只有几帧
```

所以判断很简单：**内容是录屏/幻灯片就用默认值；是真人实拍、vlog、访谈、
手持拍摄，起手就用 `--threshold 20`。** 发现压缩率还是高就继续往上调。
